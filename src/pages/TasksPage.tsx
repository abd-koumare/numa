...
export function TasksPage() {
  ...
  const [delegateTo, setDelegateTo] = useState('awa')
  ...
  const delegate = () => {
    setMessage(`Tâche déléguée à ${delegateTo === 'awa' ? 'Awa Kouassi' : delegateTo === 'mariam' ? 'Mariam Diarra' : 'Kader Yao'}.`)
    setDelegatedTaskId(null)
    setDelegationReason('')
  }
  ...
  return (
    <Box sx={{ maxWidth: 1240, mx: 'auto', px: { xs: 2, sm: 3, lg: 4 }, py: { xs: 3, md: 4 } }}>
      ...
      <Dialog open={Boolean(delegatedTaskId)} onClose={() => setDelegatedTaskId(null)} fullWidth maxWidth="sm"><DialogTitle>Déléguer la tâche</DialogTitle><DialogContent><Alert severity="info" sx={{ mb: 2 }}>La délégation conserve le demandeur initial et sera inscrite dans l’historique.</Alert><Stack spacing={2}><TextField select label="Nouveau responsable" value={delegateTo} onChange={(event) => setDelegateTo(event.target.value)}><MenuItem value="awa">Awa Kouassi · Direction Technique</MenuItem><MenuItem value="mariam">Mariam Diarra · Secrétariat général</MenuItem><MenuItem value="kader">Kader Yao · DSI</MenuItem></TextField><TextField multiline minRows={3} required label="Motif de la délégation" value={delegationReason} onChange={(event) => setDelegationReason(event.target.value)} /></Stack></DialogContent><DialogActions><Button onClick={() => setDelegatedTaskId(null)}>Annuler</Button><Button variant="contained" disabled={!delegationReason.trim()} onClick={delegate}>Confirmer la délégation</Button></DialogActions></Dialog>
    </Box>
  )
}
