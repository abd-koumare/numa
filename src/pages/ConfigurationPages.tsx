...
export function SystemSettingsPage() {
  ...
  return <Box sx={{ maxWidth: 1100, mx: 'auto', px: { xs: 2, sm: 3 }, py: 3 }}><Heading title="Paramètres système" description="Préférences générales, sécurité, fichiers et communications." action={<Button variant="contained" startIcon={<SaveOutlined />} disabled={saving || (API_DATA_ENABLED && loading)} onClick={() => void save()}>{saving ? 'Enregistrement…' : 'Enregistrer'}</Button>} />{error || actionError ? <Alert severity="error" sx={{ mb: 2 }}>{actionError || error}</Alert> : null}<Card><Tabs value={tab} onChange={(_, value: number) => setTab(value)} variant="scrollable" scrollButtons="auto" sx={{ px: 1 }}><Tab label="Général" /><Tab label="Sécurité" /><Tab label="Fichiers" /><Tab label="Notifications" /><Tab label="Internationalisation" /></Tabs><Divider /><Stack spacing={2} sx={{ p: 2.5 }}>
    {tab === 0 ? <><TextField label="Nom technique du site" value={String(values.siteName ?? '')} onChange={(event) => update('siteName', event.target.value)} /><TextField label="Page d’accueil par défaut" select value={String(values.defaultHome ?? 'dashboard')} onChange={(event) => update('defaultHome', event.target.value)}><MenuItem value="dashboard">Tableau de bord</MenuItem><MenuItem value="tasks">Mes tâches</MenuItem><MenuItem value="correspondence">Courriers</MenuItem></TextField><FormControlLabel control={<Switch checked={Boolean(values.allowBusinessAdminPublish)} onChange={(event) => update('allowBusinessAdminPublish', event.target.checked)} />} label="Autoriser les administrateurs métier à publier" /></> : null}
