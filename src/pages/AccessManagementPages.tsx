...
const allPermissions = [
  'site.manage', 'page.create', 'page.edit', 'page.publish', 'list.create', 'list.edit', 'list.delete',
  'item.create', 'item.read', 'item.edit', 'item.delete', 'item.submit', 'item.approve', 'item.reject',
  'numbering.configure', 'workflow.configure', 'signature.sign', 'audit.read', 'backup.restore', 'user.manage',
]
...
export function AddUserPage() {
  ...
  const submit = () => {
    if (!candidate) return
    const user: DirectoryUser = { ...candidate, status: 'Actif', roles: [role], groups: groupId ? [groupId] : [], lastLogin: 'Jamais' }
    addUser(user)
    navigate(`/administration/utilisateurs/${user.id}`, { replace: true })
  }

  return <Box sx={{ maxWidth: 900, mx: 'auto', px: { xs: 2, sm: 3 }, py: 3 }}><PageHeading title="Ajouter un utilisateur" description="Sélectionnez un compte existant dans l’annuaire et attribuez ses accès NUMA." /><Card><Box sx={{ p: 2.5 }}><Typography component="h2" variant="h2">Compte d’annuaire</Typography></Box><Divider /><Stack spacing={2} sx={{ p: 2.5 }}><TextField select label="Utilisateur Active Directory" value={candidateId} onChange={(event) => setCandidateId(event.target.value)}>{available.map((item) => <MenuItem key={item.id} value={item.id}>{item.name} · {item.email}</MenuItem>)}</TextField>{candidate ? <Alert severity="info"><strong>{candidate.title}</strong><br />{candidate.department} · identité synchronisée par LDAPS</Alert> : <Alert severity="warning">Tous les comptes de démonstration sont déjà ajoutés.</Alert>}<TextField select label="Rôle initial" value={role} onChange={(event) => setRole(event.target.value as UserRole)}>{roles.map((item) => <MenuItem key={item.id} value={item.id}>{item.label}</MenuItem>)}</TextField><TextField select label="Groupe NUMA" value={groupId} onChange={(event) => setGroupId(event.target.value)}><MenuItem value="">Aucun groupe</MenuItem>{groups.map((group) => <MenuItem key={group.id} value={group.id}>{group.name}</MenuItem>)}</TextField></Stack><Divider /><Stack direction="row" justifyContent="flex-end" spacing={1} sx={{ p: 2.5 }}><Button component={RouterLink} to="/administration/utilisateurs">Annuler</Button><Button variant="contained" disabled={!candidate} onClick={submit}>Ajouter à NUMA</Button></Stack></Card></Box>
}
...
export function GroupsPage() {
  ...
  const create = () => {
    ...
    addGroup({ id, name: name.trim(), description: 'Groupe métier NUMA', source: 'NUMA', memberIds: [], roleIds: [] })
    setSelectedId(id); setName(''); setDialogOpen(false)
  }
