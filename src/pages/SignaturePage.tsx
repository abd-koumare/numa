...
export function SignaturePage() {
  ...
  const correspondence = [...externalCorrespondences, ...internalCorrespondences].find((item) => item.id === id) ?? externalCorrespondences[11]
    ...
    return (
      <Box sx={{ maxWidth: 900, mx: 'auto', px: { xs: 2, sm: 3 }, py: { xs: 4, md: 7 } }}>
        <Card sx={{ p: { xs: 3, md: 5 }, textAlign: 'center' }}>
          ...
          <Typography color="text.secondary" sx={{ mt: 1 }}>La preuve est liée à la version 3 du document. Elle est maintenant visible dans la fiche et le journal d’audit.</Typography>
          <Box sx={{ maxWidth: 560, mx: 'auto', mt: 3, p: 2, bgcolor: 'background.default', borderRadius: 1, textAlign: 'left' }}>
            <MetadataRow label="Document" value={`${correspondence.reference} · version 3`} />
  ...
  return (
    <Box sx={{ maxWidth: 1400, mx: 'auto', px: { xs: 2, sm: 3, lg: 4 }, py: { xs: 3, md: 4 } }}>
      ...
      <Stack direction={{ xs: 'column', md: 'row' }} justifyContent="space-between" alignItems={{ md: 'flex-start' }} spacing={2.5} sx={{ mb: 2.5 }}>
        <Box><Typography component="h1" variant="h1">Signature électronique</Typography><Typography variant="body2" color="text.secondary" sx={{ mt: 0.75 }}><Box component="span" sx={{ fontFamily: 'IBM Plex Mono, monospace' }}>{correspondence.reference}</Box> — {correspondence.subject} · version 3</Typography></Box>
        <Stack direction={{ xs: 'column', sm: 'row' }} spacing={1}><Button component={RouterLink} to={`${basePath}/${id}`} variant="outlined">Annuler</Button><Button variant="contained" startIcon={processing ? <CircularProgress size={18} color="inherit" /> : <DrawOutlined />} disabled={!consent || processing || (level === 'digital' && certificateState !== 'valid')} onClick={() => setConfirmOpen(true)}>{processing ? 'Signature en cours…' : 'Apposer la signature'}</Button></Stack>
      </Stack>

      <Alert severity="info" icon={<LockOutlined />} sx={{ mb: 2.5 }}>Vous signez l’empreinte de la <strong>version 3</strong>. Toute modification ultérieure créera une nouvelle version non couverte par cette signature.</Alert>
          ...
          <Stack spacing={1.25} sx={{ p: 2.5 }} role="radiogroup" aria-label="Niveau de signature">
            {signatureLevels.map((item) => (
              <Card key={item.id} variant="outlined" sx={{ borderColor: level === item.id ? 'primary.main' : 'divider', bgcolor: level === item.id ? 'rgba(18,62,124,.035)' : 'background.paper', boxShadow: 'none' }}>
                <CardActionArea role="radio" aria-checked={level === item.id} onClick={() => setLevel(item.id)} sx={{ p: 2 }}>
            ...
            <Box><MetadataRow label="Document" value={`${correspondence.reference} — v3`} mono /><MetadataRow label="Utilisateur" value="Kader Yao — Configurateur" /><MetadataRow label="Type" value={signatureLevels.find((item) => item.id === level)!.title} /><MetadataRow label="Date et heure" value="Horodatage automatique" /><MetadataRow label="Empreinte" value="sha256:8f2a…c19d" mono /><MetadataRow label="Adresse IP" value="10.12.4.51" mono /></Box>
          </Stack>
        </Card>

        <Stack spacing={2.5}>
          <Card>
            <Box sx={{ p: 2.5 }}><Stack direction="row" justifyContent="space-between" spacing={1}><Typography component="h2" variant="h2">Aperçu du rendu final</Typography><Chip label="Document · v3" size="small" variant="outlined" /></Stack></Box><Divider />
          ...
          <Alert severity="warning">La signature graphique est une représentation visuelle. Pour une preuve cryptographique, sélectionnez « Signature numérique ».</Alert>
        </Stack>
      </Box>

      <Dialog open={confirmOpen} onClose={() => setConfirmOpen(false)} fullWidth maxWidth="sm">
        <DialogTitle>Confirmer la signature</DialogTitle><DialogContent><Alert severity="warning" sx={{ mb: 2 }}>Cette action est définitive pour la version 3 et sera inscrite au journal d’audit.</Alert><Typography variant="body2">Confirmez-vous la signature de l’empreinte <Box component="span" sx={{ fontFamily: 'IBM Plex Mono, monospace', fontWeight: 700 }}>sha256:8f2a…c19d</Box> avec le niveau « {signatureLevels.find((item) => item.id === level)!.title} » ?</Typography></DialogContent><DialogActions><Button onClick={() => setConfirmOpen(false)}>Annuler</Button><Button variant="contained" startIcon={<VerifiedOutlined />} onClick={confirmSignature}>Confirmer et signer</Button></DialogActions>
      </Dialog>
    </Box>
  )
}
