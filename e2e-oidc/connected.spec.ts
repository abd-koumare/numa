...
type Definition = { id: string; kind: string; slug: string; name: string; current_version: { id: string; data: any; version: number } | null; latest_version: { id: string; version: number; state: string; data: any } }
...
async function publish(client: ReturnType<typeof api>, definition: Definition) {
  return client.post(`configurations/${definition.id}/publish/`, {}, definition.latest_version.version) as Promise<Definition>
}
async function create(client: ReturnType<typeof api>, kind: string, slug: string, data: unknown) {
...
const draftData = (subject: string) => ({ registry: 'external', sender: 'Recette NUMA', origin_reference: '', received_at: new Date().toISOString().slice(0, 10), channel: 'paper', subject, direction_code: 'DT', responsible_service_code: 'DSI', priority: 'normal', confidentiality: 'standard', summary: 'Donnée de recette connectée' })

async function attachCleanDocument(request: APIRequestContext, client: ReturnType<typeof api>, item: any) {
...
test('real documents are scanned, submitted, validated, signed and audited with version and permission checks', async ({ page, request, browser }) => {
  ...
  const context = await browser.newContext(); const userPage = await context.newPage(); const tokenUser = await userToken(userPage)
  const forbidden = await request.post(`${root}/correspondences/${item.id}/sign/`, { headers: { Authorization: `Bearer ${tokenUser}`, 'If-Match': `"${item.row_version}"` }, data: { level: 'electronic-validation', document_version_id: item.documents[0].id } })
  expect(forbidden.status()).toBe(403); await context.close()
