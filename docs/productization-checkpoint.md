...
- Validation PostgreSQL du 2026-08-28 : les verrous des définitions, courriers et éléments génériques utilisent désormais `FOR UPDATE OF` sur leur table principale. Les jointures nullables nécessaires en lecture ne sont plus verrouillées ; création de courrier, transitions, signature et mise à jour générique ne déclenchent plus `FOR UPDATE cannot be applied to the nullable side of an outer join`.
...
- 2026-08-28 — Correction PostgreSQL runtime : les quatre requêtes combinant `select_for_update` et jointures nullables limitent le verrou à `self`. API/worker redémarrés, santé complète au vert et requêtes équivalentes validées en transaction puis annulées sur PostgreSQL réel.
